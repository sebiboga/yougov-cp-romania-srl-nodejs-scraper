...
This scraper extracts job listings from YouGov careers page (Romania only) and imports them to peviitor.ro.

Target: https://jobs.yougov.com/en/jobs/romania
...
1. **Start with brand** - We know the brand (e.g., "YouGov")
...
8. **Scrape new jobs** - Extract jobs from YouGov careers page (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts YouGov Careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from YouGov careers API (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape YouGov API (jobs for Romania)
    │
    ▼
...
| `tests/validate-yougov-jobs.js` | CI fast validator (HEAD only); thin CLI over `src/job-validator.js` + `solr.js` |
...
| `tests/e2e/scraper.test.js` | End-to-end tests with real YouGov API |
