...
| `src/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by both `validate-jobs.js` and `tests/validate-yougov-jobs.js`. |
...
| `tests/company.json` | Mock ANAF company data for YouGov used in unit tests |
| `tests/validate-yougov-jobs.js` | **YouGov-specific fast validator (used by CI).** HEAD requests only, hardcoded YouGov CIF. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real YouGov API, ANAF, and SOLR |
...
| `delete_request.json` | **Manual maintenance tool** — SOLR payload to delete ALL jobs for CIF 48869513. Use only when you need to wipe YouGov jobs from SOLR entirely. Run with: `curl --user "${SOLR_AUTH}" "https://solr.peviitor.ro/solr/job/update?commit=true" -H "Content-Type: application/json" -d @delete_request.json` |
...
- Full workflow: validate company (ANAF+Peviitor) → scrape YouGov → transform → upsert SOLR → generate docs/jobs.md